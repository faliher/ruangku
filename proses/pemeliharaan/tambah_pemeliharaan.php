...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "INSERT INTO data_pemeliharaan (id_pemeliharaan, id_barang_pemda, kode_barang, desk_pemeliharaan, perbaikan, tgl_perbaikan, lama_perbaikan, biaya_perbaikan)
              VALUES ('$new_id_pemeliharaan', '$id_barang_pemda', '$kode_barang', '$desk_pemeliharaan', '$perbaikan', $tgl_perbaikan, '$lama_perbaikan', $biaya_perbaikan)";

    // Eksekusi query
    if (mysqli_query($conn, $query)) {
        // Update harga_total di data_barang
        $updateHargaTotalSql = "UPDATE data_barang 
                                 SET harga_total = (harga_total + $biaya_perbaikan) 
                                 WHERE id_barang_pemda = '$id_barang_pemda'";
        mysqli_query($conn, $updateHargaTotalSql); // Eksekusi pembaruan harga total

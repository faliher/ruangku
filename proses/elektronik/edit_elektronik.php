...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id_mutasi_elektronik = generateId('ELKT', 'mutasi_elektronik', 'id_mutasi_elektronik');
    ...
    // Jika pengguna berubah, catat ke tabel mutasi_elektronik
    if ($pengguna_lama !== $pengguna) {
        $id_mutasi_elektronik = generateId('ELKT', 'mutasi_elektronik', 'id_mutasi_elektronik');
        $tgl_mutasi_elektronik = date('Y-m-d');
        $keterangan_mutasi = "Perubahan pengguna dari $pengguna_lama ke $pengguna";

        $sql_mutasi = "INSERT INTO mutasi_elektronik 
                       (id_mutasi_elektronik, id_barang_pemda, tgl_mutasi_elektronik, pengguna_lama, pengguna_sekarang) 
                       VALUES ('$id_mutasi_elektronik', '$id_barang_pemda', '$tgl_mutasi_elektronik', '$pengguna_lama', '$pengguna')";

        if (!mysqli_query($conn, $sql_mutasi)) {
            $_SESSION['error'] = "Gagal mencatat mutasi elektronik: " . mysqli_error($conn);
            header('Location: ../../frm_edit_elektronik.php?id_barang_pemda=' . $id_barang_pemda);
            exit;
        }
    }
    ...
    if (mysqli_query($conn, $sql_update)) {
        ...
        header('Location: ../../elektronik.php?id_barang_pemda=' . $id_barang_pemda);
        exit;
    } else {

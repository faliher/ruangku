...
function generateId($prefix, $table, $column)
{
    ...
    // Get the last ID and check if it's not NULL
    $last_id = $row['last_id'];

    // If there is no last_id, start from 1
    if ($last_id === NULL) {
        $numeric_part = 0;
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Initialize $surat_tugas
    $surat_tugas = "";

    // Handle file upload for Surat Tugas
    $surat_tugas = ""; // Initialize variable
    if (isset($_FILES['surat_tugas']) && $_FILES['surat_tugas']['error'] === UPLOAD_ERR_OK) {
        $surat_tugas = $_FILES['surat_tugas']['name'];
        $target_dir = "../../uploads/kendaraan/";
        $target_file = $target_dir . basename($surat_tugas);

        $allowed_types = ['jpg', 'jpeg', 'png', 'pdf', 'doc', 'docx'];
        $file_extension = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));

        if (!in_array($file_extension, $allowed_types)) {
            $_SESSION['error'] = "Only JPG, JPEG, PNG, PDF, DOC, and DOCX files are allowed.";
            header('Location: ../../frm_edit_kendaraan.php?id_barang_pemda=' . $id_barang_pemda);
            exit;
        }

        if ($_FILES['surat_tugas']['size'] > 5000000) { // Limit file size to 5MB
            $_SESSION['error'] = "File size should not exceed 5MB.";
            header('Location: ../../frm_edit_kendaraan.php?id_barang_pemda=' . $id_barang_pemda);
            exit;
        }

        if (!move_uploaded_file($_FILES['surat_tugas']['tmp_name'], $target_file)) {
    ...
    if ($pengguna_lama !== $pengguna) {
        ...
        $sql_mutasi = "INSERT INTO mutasi_kendaraan 
                       (id_mutasi_kendaraan, id_barang_pemda, tgl_mutasi_kendaraan, pengguna_lama, pengguna_sekarang, surat_tugas ) 
                       VALUES ('$id_mutasi_kendaraan', '$id_barang_pemda', '$tgl_mutasi_kendaraan', '$pengguna_lama', '$pengguna', '$surat_tugas')";

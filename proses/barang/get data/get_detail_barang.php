...
$sql_adm = "SELECT * FROM adm_pengadaan_kibb WHERE id_barang_pemda = '$id_barang_pemda'";
...
$sql_hapus = "SELECT * FROM adm_penghapusan WHERE id_barang_pemda = '$id_barang_pemda'";
$result_hapus = mysqli_query($conn, $sql_hapus);

if (mysqli_num_rows($result_hapus) > 0) {
  $row_hapus = mysqli_fetch_assoc($result_hapus);
} else {
  echo "Data barang tidak ditemukan.";
  exit;
}
